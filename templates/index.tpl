...
{block name="page"}
	...
	<tabs :track-history="true">
		<tab id="myQueue" label="{translate key="dashboard.myQueue"}"
			:badge="components.{$smarty.const.SUBMISSIONS_LIST_MY_QUEUE}.itemsMax">
		...
		{if array_intersect(array(ROLE_ID_SITE_ADMIN, ROLE_ID_MANAGER), (array)$userRoles)}
			<tab id="unassigned" label="{translate key="common.queue.long.submissionsUnassigned"}"
				:badge="components.{$smarty.const.SUBMISSIONS_LIST_UNASSIGNED}.itemsMax">
				{help file="submissions" section="unassigned" class="pkp_help_tab"}
				<submissions-list-panel v-bind="components.{$smarty.const.SUBMISSIONS_LIST_UNASSIGNED}" @set="set" />
			</tab>
			<tab id="active" label="{translate key="common.queue.long.active"}"
				:badge="components.{$smarty.const.SUBMISSIONS_LIST_ACTIVE}.itemsMax">
		...
		<tab id="archive" label="{translate key="navigation.archives"}"
			:badge="components.{$smarty.const.SUBMISSIONS_LIST_ARCHIVE}.itemsMax">
		...
		<tab id="certificates" label="Meus certificados">
			{include file="file:plugins/generic/reviewCertificate/templates/reviewerSubmissions.tpl"}
		</tab>
	</tabs>
{/block}
